    ...
    if (typeof userPrompt === "string") {
      this.messages.push({
        ...
        content: [
          {
            "type": "text",
            "text": "请描述图里出现的场景，内容，物品，和出现的文字"
          },
          {
            "type": "image_url",
            "image_url": {
                "url" : "http://morethanchat.club/server/uploads/1724801455774.jpg"
            }
          }
        ]
      });
...
class P5GLM4V extends P5LLM {
  ...
  preSend() {
    ...
    let exactMessages = this.messages.map(message => {
    ...
    if (this.systemPrompt || this.systemPrompt.length > 0) {
      exactMessages.unshift({
        role: "system",
        content: this.systemPrompt,
      });
    }
    console.log(exactMessages);
    return exactMessages;
  }
